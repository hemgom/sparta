...
import calculator.level04.validation.CalculationResultValidator;
...
public class Level04 {
    ...
    // 연산결과를 검증하기 위해 CalculationResultValidator 객체 사용
    ...
    public boolean start(Scanner sc) {
        ...
        if (inputValidator.isExitCommand(input)) return false;

        // 입력 값의 유효성을 검증해 유효하지 않다면 예외가 발생, 발생한 예외를 처리
        // 예외처리 후 true 를 반환해 계산기 처음으로 돌아가 다시 입력부터 시작
        try {
            inputValidator.checkValidCalculation(input);
        ...
        // 연산 결과가 NaN 또는 Infinity 일 경우의 예외를 처리
        double result;
        try {
            result = cal.calculate(findByOperator(operation[1]));   // 연산 수행
            calculationResultValidator.checkDivideByZero(result);
        } catch (IllegalArgumentException | DivideByZeroException e) {
            System.out.println(e.getMessage());
            return true;
        }
